...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="app-shell">
    <header className="site-header"><div className="header-inner"><Link to="/" className="brand" aria-label="Archer home"><span className="brand-mark">A</span><span>Archer</span></Link><nav className={menuOpen ? "nav-links nav-open" : "nav-links"}><NavLink to="/" end>{t("findWork")}</NavLink><a href="#how-it-works">{t("howItWorks")}</a><a href="#talent">{t("forTalent")}</a></nav><div className="header-actions"><label className="language-switcher"><span>{t("language")}</span><select aria-label={t("language")} value={language} onChange={(event) => setLanguage(event.target.value as "en" | "my")}><option value="en">{t("english")}</option><option value="my">{t("burmese")}</option></select></label><button className="icon-button mobile-menu" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle menu"><Menu size={20} /></button><Link to="/login" className="header-login">{t("signIn")}</Link><Link to="/login?mode=register" className="button button-primary header-cta">{t("postProject")} <ArrowUpRight size={16} /></Link></div></div></header>
    <main>{children}</main>
